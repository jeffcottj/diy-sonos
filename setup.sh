...
init_backup_snapshot_dir() {
    if [[ -z "${BACKUP_SNAPSHOT_DIR:-}" ]]; then
        if [[ "${BACKUP_REQUESTED:-0}" -eq 1 ]]; then
            BACKUP_SNAPSHOT_DIR="/var/backups/diy-sonos/$(date +%Y%m%d-%H%M%S)"
        elif [[ -n "${DIY_SONOS_BACKUP_DIR:-}" ]]; then
            BACKUP_SNAPSHOT_DIR="$DIY_SONOS_BACKUP_DIR"
        fi
    fi

    if [[ -n "${BACKUP_SNAPSHOT_DIR:-}" ]]; then
        mkdir -p "$BACKUP_SNAPSHOT_DIR"
        echo "Backup snapshots enabled: $BACKUP_SNAPSHOT_DIR"
    fi
}
...
write_generated_config() {
    ...
    if [[ -n "$client_ips_csv" ]]; then
        clients_yaml=""
        IFS=',' read -r -a client_ips <<< "$client_ips_csv"
        local client_ip
        for client_ip in "${client_ips[@]}"; do
            client_ip="${client_ip//[[:space:]]/}"
            [[ -n "$client_ip" ]] || continue
            clients_yaml+="  - ip: \"${client_ip}\""$'\n'
        done
    fi

    snapshot_file "$GENERATED_CONFIG"
...
run_init_mode() {
    init_backup_snapshot_dir
    local preset="${PRESET:-advanced}"
    local role="${ROLE:-}"
    local server_ip="${SERVER_IP:-}"
    local client_ips_csv="${CLIENT_IPS:-}"
    ...
    if [[ "$preset" != "basic" && "$preset" != "advanced" ]]; then
        echo "Error: --preset must be 'basic' or 'advanced'" >&2
        exit 1
    fi
    ...
    if [[ "$preset" == "basic" ]]; then
        while [[ -z "$client_ips_csv" ]]; do
            client_ips_csv="$(prompt_with_default "Client IP(s), comma-separated" "")"
            client_ips_csv="${client_ips_csv//[[:space:]]/}"
            if [[ -z "$client_ips_csv" ]]; then
                echo "Please enter at least one client IP."
            fi
        done
    fi
    ...
    if [[ -z "$codec" ]]; then
        if [[ "$preset" == "basic" ]]; then
            ...
            codec="$(prompt_with_default "Snapserver codec (flac/pcm)" "flac")"
        fi
    fi
    ...
    if [[ -z "$buffer_ms" ]]; then
        if [[ "$preset" == "basic" ]]; then
            ...
            buffer_ms="$(prompt_with_default "Snapserver buffer_ms" "1000")"
        fi
    fi
